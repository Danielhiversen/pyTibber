    ...
    def test_home(self):
        ...
        for home in self.tibber.get_homes(only_active=False):
            ...
            if home.home_id == "c70dcbe5-4485-4821-933d-a8a86452737b":
                ...
                self.assertTrue(
                    isinstance(home.current_price_info.get("energy"), (float, int))
                )
                self.assertTrue(
                    isinstance(home.current_price_info.get("startsAt"), str)
                )
                self.assertTrue(
                    isinstance(home.current_price_info.get("tax"), (float, int))
                )
                self.assertTrue(
                    isinstance(home.current_price_info.get("total"), (float, int))
                )
...
class TestTibberInvalidToken(unittest.TestCase):
    ...
    def test_tibber(self):
        ...
        async def run():
            self.assertFalse(
                await self.tibber.send_notification("Test tittle", "message")
            )
